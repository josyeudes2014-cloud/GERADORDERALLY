...
export function subscribeAuth(callback:(user:User|null)=>void){return supabase.auth.onAuthStateChange((_event,session)=>callback(session?.user??null)).data.subscription.unsubscribe;}
